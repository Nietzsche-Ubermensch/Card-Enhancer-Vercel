# CardCrop AI Suite — iPhone Implementation Blueprint

Status: v1.0 — grounded in an audit of this repository (`billcooperneverdies-cmyk/Card-Enhancer`)
...
- **No provider keys in the bundle** (`GEMINI/OPENAI/XAI/OPENROUTER/VENICE`): the app authenticates to the CardCrop backend only; the backend brokers any AI provider calls server-side. (Audit note: no such keys exist in the repo today; this rule binds the new backend work.)
