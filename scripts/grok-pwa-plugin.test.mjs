...
// Isolate from this workspace bake (Card Enhancer Suite + public/og.jpg).
const ISO_CWD = mkdtempSync(join(tmpdir(), "grok-pwa-iso-"));
function pwa(html, ctx = {}) {
  return injectGrokPwaHead(html, { cwd: ISO_CWD, ...ctx });
}
function stream(ctx = {}) {
  return createHeadInjector({ cwd: ISO_CWD, ...ctx });
}


test("injects before </head>", () => {
  const out = pwa("<html><head><title>x</title></head><body></body></html>");
...
test("injects the extensions script without a project id", () => {
  const out = pwa("<html><head></head></html>", {
    appName: "Demo",
    projectId: "",
  });
...
test("does not duplicate grok:app_id", () => {
  ...
  const once = pwa("<html><head></head></html>", ctx);
  const twice = pwa(once, ctx);
...
test("platform chrome overwrites share-card metas and always sets og:title", () => {
  ...
  const out = pwa(html, { appName: "Wild Race" });
...
test("does not duplicate twitter:card or og:title", () => {
  const once = pwa("<html><head><title>Hello World</title></head></html>");
  const twice = pwa(once);
  assert.equal(once, twice);
...
test("does not emit x:game:image without a public host or banner", () => {
  const noHost = pwa("<html><head></head></html>", {
    site: { banner: "/x-banner.jpg" },
  });
  assert.doesNotMatch(noHost, /x:game:image/);
  const noBanner = pwa("<html><head></head></html>", {
    host: "wild-race.grok.me",
...
test("published VITE_PUBLIC_HOSTNAME wins over request Host for og:image", () => {
  ...
  try {
    const vercelHost = pwa("<html><head><title>RACK</title></head></html>", {
    ...
    const otherPublicHost = pwa("<html><head><title>RACK</title></head></html>", {
...
test("vercel Host without a public hostname emits no og:image", () => {
  ...
  try {
    const out = pwa("<html><head><title>RACK</title></head></html>", {
      host: "01a020b6-803a-71a2-bb47-e2bec57eb9a2-662k8x1l1-xai-org.vercel.app",
      site: { title: "RACK", card: "custom" },
    });
    assert.doesNotMatch(out, /property="og:image"/);
...
test("emits og:image for a public host and prefers a custom card", () => {
  const placeholder = pwa("<html><head></head></html>", {
    appName: "Wild Race",
    host: "wild-race.grok.me",
    site: { title: "Wild Race" },
  });
  ...
  const custom = pwa("<html><head></head></html>", {
    appName: "Wild Race",
    host: "wild-race.grok.me",
...
test("placeholder og:image appends site.color when it is 6-digit hex", () => {
  const themed = pwa("<html><head></head></html>", {
    host: "wild-race.grok.me",
  ...
  const invalid = pwa("<html><head></head></html>", {
    host: "wild-race.grok.me",
...
test("injects into documents with no head element", () => {
  const out = pwa("<html><body>hi</body></html>", { appName: "Solo" });
...
test("streaming injector matches </HEAD> case-insensitively", () => {
  const injector = stream({ appName: "Wild Race" });
  const chunks = [
...
test("is idempotent", () => {
  const once = pwa("<html><head></head></html>");
  const twice = pwa(once);
  assert.equal(once, twice);
});

test("uses the app name in the injected title tag", () => {
  const out = pwa("<html><head></head></html>", { appName: "Wild Race" });
