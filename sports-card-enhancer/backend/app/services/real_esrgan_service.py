...
class RealESRGANService:
    ...
    def upscale_with_fallback(
        ...
        Args:
            ...
            outscale: Override output scale (defaults to model's native scale)
        ...
        """
        scale = int(outscale) if outscale else fallback_scale
        try:
            upsampler = self.load_model(model_name)
            result, _ = upsampler.enhance(image, outscale=outscale if outscale else upsampler.scale)
            return result, True
